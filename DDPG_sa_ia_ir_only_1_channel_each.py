    ...
    for i_loop in range(4):
        ...
        for i_episode in range(episode):
            ...
            if (i_episode+1) % 50 == 0 or i_episode == 0:
                ...
                dataframe.to_csv("./result/ddpg_actor_loss_sa_ia_ir_seed_"+str(i_seed)+"_loop_"+str(i_loop)+"_episode_"+str(i_episode)+".csv", index=False,sep=',')
                dataframe=pd.DataFrame({'bitrate':critic_loss_history})
                dataframe.to_csv("./result/ddpg_critic_loss_sa_ia_ir_seed_"+str(i_seed)+"_loop_"+str(i_loop)+"_episode_"+str(i_episode)+".csv", index=False,sep=',')
 
        dataframe=pd.DataFrame({'bitrate':system_ave_bitrate_history})
        dataframe.to_csv("./result/ddpg_bitrate_sa_ia_ir_seed_"+str(i_seed)+"_loop_"+str(i_loop)+".csv", index=False,sep=',')
        dataframe=pd.DataFrame({'reward':reward_ave_history})
        dataframe.to_csv("./result/ddpg_reward_sa_ia_ir_seed_"+str(i_seed)+"_loop_"+str(i_loop)+".csv", index=False,sep=',')